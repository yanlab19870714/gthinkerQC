...
#include "../divide_conquer_version/kcore.h"
...
int gnmax_size;
int gnmin_deg;
VertexID max_v; //max degree v. Don't need to spawan v's neighbor
...
//	void operator=(const Graph &obj);
	...
	void ClearGraph();
	void TranGraph(map<VertexID, kc_value> & gmap, vector<vector<VertexID> >& adj_list);
	int LoadGraph(vector<vector<VertexID> >& adj_list);
	void GenLevel2NBs();
	void DestroyGraph();
...
//	bool ForceGenCondGraph(VERTEX* pvertices, int nclique_size, int num_of_cands, int num_of_tail_vertices, Graph& split_g);
	...
	friend obinstream& operator>>(obinstream& m, Graph& g)
	{
		...
		g.index2id = new int[mnum_of_vertices];
		for(int i = 0; i < mnum_of_vertices; i++)
			m >> g.index2id[i];
...
void Graph::TranGraph(map<VertexID, kc_value> & gmap, vector<vector<VertexID> >& adj_list)
{
	int num_of_vertices = gmap.size();
	map<VertexID, int> id2index_map;
	index2id = new int[num_of_vertices];
	int index = 0;
	for(auto it = gmap.begin(); it != gmap.end(); ++it)
	{
		VertexID vid = it->first;
		//root v has already been put at the beginning here
		index2id[index] = vid;
		id2index_map[vid] = index;
		index++;
	}
	//translate the vid into vec<vec<> > adj_list
	index = 0;
	for(auto it = gmap.begin(); it != gmap.end(); ++it)
	{
		...
		for(VertexID nb : nbs)
		{
			auto nb_it = id2index_map.find(nb);
			if(nb_it != id2index_map.end())
				adj_list[index].push_back(nb_it->second);
		}
		index++;
	}
	gmap.clear();
...
int Graph::Cliques(map<VertexID, kc_value> & gmap, FILE *gfpout)
{
...
//	VertexID root = 0;
	...
	TranGraph(gmap, adj_list);
	num_of_vertices = LoadGraph(adj_list);
	...
	//pvertices will shrink in the iterative Expand().
...
//update the degree based on removed v. I will go to neigbor's neigbor, just like our kcore-prune
	...
	/*
	//find the max degree v and set its neighbor bto_be_extended = false
	nmax_deg = 0;
	nmaxdeg_vertex = 0;
	...
	//don't need to set root's nbs bto_be_extended to false as we go with root already in
	//???whether it gonna expand to 2 directions: should be restricted outside the gthinker.
	//the root already at the head
	/*
	onevertex = pvertices[root];
	pvertices[root] = pvertices[0];
	pvertices[0] = onevertex;
	*/
...
//should revise as our S will not be empty.
	if(num_of_cands>=gnmin_size)
	{
		...
		ExpandOnce(pvertices, 0, num_of_cands, 0, gfpout);
...
int Graph::LoadGraph(vector<vector<VertexID> >& adj_list) // create 1-hop neighbors
{
	int num_of_vertices, nbuf_size, nvertex_no, nbuf_pos;
	int nsize, nlist_len, i;

	num_of_vertices = adj_list.size();
	nbuf_size = 0; // ---> all transactions are saved in one buffer.... (concat adj-lists)
	//translate gmap to padj_lens, mppadj_lists...
	//check buffer size need to prepare
	...
	for(auto adj : adj_list)
	{
		...
		for(VertexID v: adj)
		{
			if(v!=nvertex_no) // remove self-loop
				mppadj_lists[nvertex_no][nlist_len++] = v;
		}
		nbuf_pos += nlist_len;
...
//	//todo ptransaction is just like the gthinker's graph, which hold the adj_list info
//	//should convert to gthinker mode
//	//translate ptransaction to padj_lens, mppadj_lists...
//	ptransaction = pdata->getNextTransaction();
//	while(ptransaction)
//	{
//		//nmax_deg only used for printing, don't need in gthinker code
