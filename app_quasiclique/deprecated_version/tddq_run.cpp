...
public:
	...
	void graph_shrink(QCSubgraph & pre_gs, vector<QCVertex*> & cand, QCSubgraph & g,
			QCSubgraph & shrinked_G, vector<VertexID>& cand_id_vec, QCHopSubgraph & new_g_2hop){
		//Input:
			...
			for(VertexID adj : it->value)
			{
				if(remain_set.find(adj) != remain_set.end())
				{
					...
					for(VertexID nb_2hop : nb_it->value)
						if(remain_set.find(nb_2hop) != remain_set.end())
...
//		sort(cand_deg.begin()+1, cand_deg.end(),comp);
	...
	bool tddq_QCQ(QCSubgraph & gs, QCSubgraph & g, vector<QCVertex*> & cand_exts,
			ofstream & fout, chrono::steady_clock::time_point & init_time, QCHopSubgraph& g_2hop){
				...
				if(exe_time > TIME_THRESHOLD){
					...
					if(!ext_prune && new_cand_size + new_gs_size >= min_size){
						...
						graph_shrink(new_gs, new_cand, g, t->subG,
								t->context.cand_id_vec, t->context.g_2hop);
						t->context.round = 3;
						add_task(t);
					}
					...
					if(!ext_prune && new_cand_size + new_gs_size >= min_size){
								...
								&& g.vertexes.size() > COMPACT_SIZE)
						{
							...
							bhas_super_qclq = tddq_QCQ(new_gs, new_g, new_cand, fout, init_time, new_g_2hop);
						} else {
							bhas_super_qclq = tddq_QCQ(new_gs, g, new_cand, fout, init_time, g_2hop);
						}
    ...
    virtual bool compute(SubgraphT & g, ContextT & context, vector<VertexT *> & frontier)
    {
    	...
    	else if(context.round == 2) //context == 2
    	{
			...
			QCHopSubgraph& g_2hop = context.g_2hop;
			for(auto it = g_map.begin(); it != g_map.end(); ++it){
				QCVertex v;
				QC2Hop v2hop;
			...
			sort_deg(cand_exts, context.gs, context.g_2hop);
			auto init_time = chrono::steady_clock::now();

			QCSubgraph new_g;
			QCHopSubgraph new_g_2hop;
			compact_graph(context.gs, cand_exts, g, new_g, new_g_2hop);

    		tddq_QCQ(context.gs, new_g, cand_exts, fout, init_time, new_g_2hop);
    		return false;
    	}
    }
};
