...
int SPLIT_THRESHOLD; // if |Xcand| >= DIVIDE_THRESHOLD, split into subtasks
...
public:
    ...
    virtual bool compute(SubgraphT & g, ContextT & context, vector<VertexT *> & frontier)
    {
    	...
    	else if(context.round == 2) //context == 2
    	{
    		...
    		QCSubgraph & gs = context.gs;
    		//???? do we need to change the threshold
    		if(cand_size <= SPLIT_THRESHOLD){
