...
public:
	...
	void expand(vector<CliqueVertex> & vertexes, vector<int> & N, VSet & Q, VSet & Qmax,
			CliqueValue & context, chrono::steady_clock::time_point & init_time) //vertexes = R in the paper
	{
				...
				if(!Rp_pts.empty())
				{
					...
					if(Q.size() + colorN.back() > Qmax.size()) // check before expand
					{
						...
						{ // split tasks
							...
							expand(Rp, colorN, Q, Qmax, context, init_time);
						}
					}
				}
    ...
    virtual bool compute(SubgraphT & g, ContextT & context, vector<VertexT *> & frontier)
    {
		...
		if(vertices.size() > 0) //====> note that "g" does not contain "root" (stored in context), can be empty
		{
			...
			VSet Q;
			expand(context.vvec, context.N, Q, Qmax_dummy, context.context, init_time);
		}
...
int main(int argc, char* argv[])
{
    ...
    WorkerParams param;
    if(argc != 6){
    	cout<<"arg1 = input path in HDFS, arg2 = number of threads, arg3 = split-time threshold, arg4 = BIGTASK_THRESHOLD, arg5 = VNUM_ALLOWED_BEFORE_SPLIT"<<endl;
    	return -1;
    }
    ...
    VNUM_ALLOWED_BEFORE_SPLIT = atoi(argv[5]);
